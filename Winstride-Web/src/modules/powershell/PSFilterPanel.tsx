...
import { type PSFilters, type FilterState, getDefaultPSFilters } from './shared/filterTypes';
...
export default function PSFilterPanel({ filters, onFiltersChange, availableMachines }: Props) {
  ...
  /* ---- Event counts ---- */
  const eventVisibleCount = countVisible(PS_EVENT_IDS, filters.eventFilters);
    ...
    <div className="bg-[#0d1117] border border-[#21262d] rounded-xl p-4 space-y-3">
        ...
        right={
          <div className="flex items-center gap-1.5">
            <span className="text-[10px] tabular-nums text-gray-600">
              {eventVisibleCount}/{PS_EVENT_IDS.length}
            </span>
      ...
      <CollapsibleSection title="Level">
        <div
          onClick={(e) => {
            e.stopPropagation();
            updateFilter('levelFilter', filters.levelFilter === 'all' ? 'warning-only' : 'all');
          }}
        ...
        >
          <div
            className={`relative w-7 h-4 rounded-full transition-colors flex-shrink-0 ${
              filters.levelFilter === 'warning-only' ? 'bg-[#f0883e]' : 'bg-[#30363d]'
            }`}
          >
            <span
              className={`absolute top-[2px] left-[2px] w-3 h-3 rounded-full bg-white shadow transition-transform ${
                filters.levelFilter === 'warning-only' ? 'translate-x-[12px]' : 'translate-x-0'
              }`}
            />
          </div>
          <span className="text-[11px] text-gray-300">Suspicious only (Warning level)</span>
        </div>
      </CollapsibleSection>
